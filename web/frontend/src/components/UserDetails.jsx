    ...
    return (
        <div className='flex text-xl rounded-md w-full bg-slate-400 py-2 text-black font-semibold shadow-inner shadow-black'>
            ...
            <div className='  w-[20%] flex justify-center  border-gray-700 items-center gap-4'>
                ...
                {handleToggleIsActive === false ? null:  <input type="checkbox" className="z-10 toggle toggle-success rounded-full " onChange={() => handleToggleIsActive?.mutate(user.id)}  value={user.is_active} disabled={handleToggleIsActive?.isLoading}/>}
